...
let opponent = huskyGames2016.map(extractOpponent);
...
for(var i=0;i<lostarray.length;i++){
  if(lostarray[i].home=="UW"){
    uwscore = lostarray[i].home_score;
    opponentscore = lostarray[i].opponent_score;
  }
  else{
    uwscore = lostarray[i].opponent_score;
    opponentscore = lostarray[i].home_score;
  }
  console.log(extractOpponent(lostarray[i]),"at UW,",opponentscore,"to",uwscore);
}
...
function fumbleornot(game){
if(game.fumbles>=1)
return true;
else 
return false;
}
let fumblearray = huskyGames2016.filter(fumbleornot);
...
if(game1.hasOwnProperty("passing_yards")==false)
return game2;
else if(game2.hasOwnProperty("passing_yards")==false)
return game1;
...
// makeCombinedFilter(functiona, functionb){
 // let combinFilter= function(game){
...
//let fumbledAndLostFilter = makeCombinedFilter(huskiesLost, fumbleornot);
...
//let lostwithfumble = huskyGames2016.fumbledAndLostFilter();
//console.log(huskyGames2016[lostwithfumble]);
